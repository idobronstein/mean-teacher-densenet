...
class Model:
    ...
    def __init__(self, run_context=None):
        ...
        (
            ...
            growth_rate=self.DENSENET_PARAM['growth_rate'],
            total_blocks=self.DENSENET_PARAM['total_blocks'],
            depth=self.DENSENET_PARAM['depth'],
            bc_mode=self.DENSENET_PARAM['bc_mode'],
            reduction=self.DENSENET_PARAM['reduction'])
...
def inference(inputs, is_training, ema_decay, input_noise, student_dropout_probability, teacher_dropout_probability,
              normalize_input, flip_horizontally, translate, growth_rate, total_blocks, depth, bc_mode, reduction):
