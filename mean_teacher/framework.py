...
def ema_variable_scope(name_scope_name, var_scope, decay=0.999):
    ...
    def use_ema_variables(getter, name, *_, **__):
        # the moving mean and moving_variance are calc in place so they will be update in the em network 
        if 'moving_mean' not in name and 'moving_variance' not in name:
            assert name in original_trainable_vars, "Unknown variable {}.".format(name)
            return ema.average(original_trainable_vars[name])
        else: 
            graph = tf.get_default_graph()
            return [i for i in graph.get_collection('variables') if name in i.name][0]
